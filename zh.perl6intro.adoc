...
* 从 https://rakudostar.com/latest/star/macos 下载最新的安装器(.dmg 后缀的文件)

.Windows
. 从 或 下载最新的安装器(.msi 后缀的文件) +
. 如果你的系统架构是 64-bit, 可以从 https://rakudostar.com/latest/star/win64 下载最新的安装器(.msi 后缀的文件); 如果是 32-bit, 则从 https://rakudostar.com/latest/star/win32 下载最新的安装器(.msi 后缀的文件）。
...
在 Perl 6 中使用  *twigil* 使得封装很便利。 +
twigil 是次级的 _魔符(sigil)_ 。它们存在于魔符和属性名之间。 +
有两个 twigil 用在类中:
